...
export default function AiResponseButton({
  ...
  return (
    <div className="space-y-2 rounded-lg p-2 flex flex-col justify-center">
      {isOpen && (
        <details open={isOpen} className="group border border-gray-200 rounded-lg shadow-sm transition-all duration-300 open:shadow-lg open:border-gray-300">
      ...
      {!isOpen && (
        <button
          ...
          onClick={handleGenerateResponse}
        >
          {isOpen ? "Create again" : `Create ${options[option]}`}
        </button>
      )}
    </div>
  );
